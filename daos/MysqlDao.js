...
exports.findById = function(res, tableName,id){
    ...
    var connection = mysql.createConnection(constant.mysqlInfo);
    var sql_findbyid = constant.sql_script.sql_findById_isactive.replace('#table', tableName);
    var sql_param = [id];
    connection.connect(function(err,connect){
        if(err){
...
exports.removeById = function(res, tableName,id){
    ...
    var connection = mysql.createConnection(constant.mysqlInfo);
    var sql_removebyid = constant.sql_script.sql_removeById.replace('#table', tableName);
    var sql_param = [id];
    connection.connect(function(err,connect){
        if(err){
 ...
 * @ param : id - id of object
 */
exports.updateById = function(res, tableName, updatePerson, updateId){
    console.log(" +++ " + "DAO update by id "+ updateId +" : " + tableName);
    var connection = mysql.createConnection(constant.mysqlInfo);
    var sql_updateById = constant.sql_script.sql_updateById.replace('#table', tableName);
